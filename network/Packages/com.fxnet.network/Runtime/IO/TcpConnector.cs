...
namespace FxNet.IO
{
    ...
    public class TcpConnector : ConnectorSocket
    {
        ...
        public TcpConnector ConnectWithSocket(Socket socket, IPEndPoint remoteEp, ErrorCode error, TextWriter? output)
        {
            ...
            catch (Exception ex) { Debug.LogWarning($"TcpConnector:ConnectWithSocket {ex.Message}"); }
        ...
        private void OnConnected(TextWriter? output)
        {
            ...
            catch (Exception ex) { Debug.LogWarning($"TcpConnector:OnConnected {ex.Message}"); }
        ...
        {
            Debug.LogError($"TcpConnector {NativeSocketHandle?.Handle} error: {error.GetWhat()}");
        }

        public override void OnClose(TextWriter? output)
        {
            Debug.Log($"TcpConnector {NativeSocketHandle?.Handle} closed");
        }
